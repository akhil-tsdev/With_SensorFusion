...
/* Keep track of the timestamp when we received the packet */
static uint32_t rfcbuf_ts[RFCB_SIZE + 1] = {0};
...
/* Function to push a Satellite packet value into the queue
 * Input val : element to be inserted into the queue
 * Output    : -1 if insertion fails, 0 for successful insertion
 * 
 * WARNING for SAT we only copy important info!!
 */
err_t rfcbufPushSatPkt(rfcbuf_t *cb, RFCB_TYPE val, uint32_t ts)
{
	if (cb == NULL) {
		return E_NO_RESOURCE;
	}
 	...
 	/* WARNING for SAT we only copy important info */
	cb->A[cb->tail].packet_type = sth_packet;
	cb->A[cb->tail].packet.sth.header.sender = val.packet.sth.header.sender;
	cb->A[cb->tail].packet.sth.satellite_to_hub_payload.header.end_of_roundrobin = \
		val.packet.sth.satellite_to_hub_payload.header.end_of_roundrobin;
	cb->A[cb->tail].timestamp = val.timestamp;
		
	rfcbuf_ts[cb->tail] = ts;
 ...
 * Output    : -1 if insertion fails, 0 for successful insertion
 */
err_t rfcbufPush(rfcbuf_t *cb, RFCB_TYPE val, uint32_t ts)
{
	if (cb == NULL) {
		return E_NO_RESOURCE;
	}
 ...
 */
err_t rfcbufPop(rfcbuf_t *cb, RFCB_TYPE *pkeyval, uint32_t* ts)
{
	if (cb == NULL) {
		return E_NO_RESOURCE;
	}
    if(rfcbufIsEmpty(cb) == FALSE) /* make sure queue is NOT empty */
    {
		...
		*ts = rfcbuf_ts[cb->head % RFCB_SIZE];
