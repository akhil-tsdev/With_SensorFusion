...
uint32_t current_command = NORDIC_INVALID;
...
static void set_spi_buffer() {
	...
	const uint32_t max_sat_idx = MAX_SAT_SENSORS - 1;
   ...
   * Suppose we have max sat of 14, so max_sat_idx will be 13.
	...
	for(int i=0;i<MAX_SENSOR_RECORDS_PER_PACKET; ++i) {
		...
		tx_data.record[i].satellite_id = sat_idx;
...
uint32_t spi_slave_example_init(void)
{
    ...
    APP_ERROR_CHECK(err_code);
		...
		for(int i=0;i<MAX_SENSOR_RECORDS_PER_PACKET; ++i) {
			tx_data.record[i].timestamp = INVALID_TIMESTAMP;
			tx_data.record[i].satellite_id = INVALID_TIMESTAMP;
			tx_data.record[i].data.quat_w = 0xaa;
			tx_data.record[i].data.quat_x = 0xbb;
			tx_data.record[i].data.quat_y = 0xcc;
			tx_data.record[i].data.quat_z = 0xdd;
		}

//first one seems to be lost, let's not set it yet		set_spi_buffer();
		padded_nordic_to_hub_t tmp = padded_tx;
