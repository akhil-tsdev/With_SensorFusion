...
uint32_t cur_sat_ids[MAX_SAT_SENSORS] = {1};
...
static void cue_next_satellite(void) {
	...
	if(ct_rtc_sendings_left) {
		...
		packet.payload.hub_to_satellite_payload.rtc_value = timestamp_to_send;
	...
	packet.payload.hub_to_satellite_payload.command = command;
	packet.payload.hub_to_satellite_payload.recipient = cur_sat_ids[target_sat_idx];
	packet.payload.hub_to_satellite_payload.last_seen_crc16 = last_received_crc16[target_sat_idx];
	packet.payload.hub_to_satellite_payload.command_crc32 = 0;
	uint32_t payload_crc32 = crc32(0xFFFFFFFF, &packet.payload.hub_to_satellite_payload,
	                               sizeof(packet.payload.hub_to_satellite_payload));
	packet.payload.hub_to_satellite_payload.command_crc32 = payload_crc32;
#if PRINT_DEBUG
#if SLOW_DEBUG
	char msg[200] = {0};
	snprintf(msg,199,"\r\nSending %x to #%d (acking CRC %04.04x) ",
		packet.payload.hub_to_satellite_payload.command,
		packet.payload.hub_to_satellite_payload.recipient,
		packet.payload.hub_to_satellite_payload.last_seen_crc16
	);
	debug_putstring(msg);
#endif
#endif
    ...
    net_init(process_packet, nack_warning, STD_MTS_CHANNEL, STD_STM_CHANNEL);
	}
}
...
void process_good_packet(uint8_t satellite_id, sth_payload_t *payload) {
	uint8_t sat_idx;
	...
	if(payload->header.timestamp != INVALID_TIMESTAMP)
	{
		...
		for(int i=0;i<SENSOR_RECORDS_PER_NET_PACKET;++i) {
			...
			outbound_datum.satellite_id = satellite_id;
...
bool is_response_packet_real(packet_t packet) {
	rs_callback_ct++;
	...
	// short packets.
	uint16_t pattern = packet.payload.satellite_to_hub_payload.diagnostic_pattern1;
	pattern =  (pattern<<8) +  packet.payload.satellite_to_hub_payload.diagnostic_pattern2;
	...
	sth_payload_t payload = packet.payload.satellite_to_hub_payload;
...
bool satellite_has_advanced_between_packets(sth_payload_t *old_payload, sth_payload_t *new_payload) {	
	...
	uint16_t new_crc16 = crc16_compute((uint8_t*)(new_payload),sth_payload_size(new_payload),0);
	uint16_t old_crc16 = crc16_compute((uint8_t*)(old_payload),sth_payload_size(old_payload),0);
...
void process_response(packet_t packet) {
	uint8_t sat_idx;
	
	valid_rs_ct++;
	sth_payload_t payload = packet.payload.satellite_to_hub_payload;
	
	sat_idx = get_sat_idx(packet.header.sender);
	if (sat_idx == INVALID_SAT_IDX) {
		return;
	}
	...
	if(satellite_has_advanced_between_packets(
		...
		total_rows_received++;
		process_good_packet(packet.header.sender, &last_payload[sat_idx]);
	}
	
	last_payload[sat_idx] = payload;
	last_received_crc16[sat_idx] = crc16_compute((uint8_t*)(&payload),sth_payload_size(&payload),0);
}
...
void talk_with_one_satellite(void) {
	...
	while(!received_real_packet && main_loop_cycle_counter != timeout_position && !packet_was_nacked) {
		...
		if(packet_waiting) {
			if(is_response_packet_real(packet_to_process)) {
				// The wait is over.
	...
	if(received_real_packet) {
		...
		process_response(packet_to_process);
		packet_waiting = false;
	} else {
		requests_timed_out++;
	}
...
#endif
#if PRINT_DEBUG
			if(main_loop_cycle_counter%1000 == 0 || total_rows_received % 1000 == 1 || control_button_pressed()) {
				...
				snprintf(report,109,"\r\n%d TX, %d%% RX'd (o/w %d%% were acked). %d%% of RXs valid.\r\n%d packets (%d OOT), %d rows.",
														diag_tx_attempt_ct,
														(diag_rx_ct*100/(diag_tx_attempt_ct)),
														(diag_rx_after_ack_ct*100/diag_rx_ct),
														(valid_rs_ct*100/diag_rx_ct),
														total_rows_received, ct_out_of_turn, rows_seen
				);
				debug_putstring((uint8_t*)report);
				...
				debug_putstring((uint8_t*)report);
				debug_putstring((uint8_t*)"\r\nBy frame [");
				for(int qq=0;qq<=CYCLES_TO_TIMEOUT;++qq) {
					snprintf(report,99,"%d, ",time_to_fill_histogram[qq]);
					debug_putstring((uint8_t*)report);
				}
				snprintf(report,99,"X, %d]",requests_timed_out);
				debug_putstring((uint8_t*)report);
				debug_putstring((uint8_t*)"\r\nBy source [");
