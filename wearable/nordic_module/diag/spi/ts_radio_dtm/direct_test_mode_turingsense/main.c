 ...
 */
#if PRINT_DEBUG
#define MAX_ITERATIONS_NEEDED_FOR_NEXT_BYTE 100
#else
...
static void uart_init(void)
{
	...
	simple_uart_config(RTS_PIN_NUMBER, TX_PIN_NUMBER, CTS_PIN_NUMBER, RX_PIN_NUMBER, HWFC, BITRATE);
#else
    ...
    for (;;)
    {
...
#if PRINT_DEBUG
				simple_uart_put(rx_byte);
				snprintf(report,109,"\r\nAccepted command: 0x%x",dtm_cmd_from_uart);
        ...
        if (dtm_event_get(&result))
        {
            ...
            // Transmit MSB of the result.
					  tx_byte = (result >> 8) & 0xFF;
					  if (tx_byte == 0) {
							simple_uart_put('0');
						} else if (tx_byte == 1) {
							simple_uart_put('1');
						} else {
							simple_uart_put(tx_byte);
						}
