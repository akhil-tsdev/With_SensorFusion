...
void ts_init_ts_radio_dtm_command(void)
{
	 ...
	 g_ts_radio_dtm_power_level = UESB_TX_POWER_4DBM;
}
...
void ts_radio_dtm_command_help(void)
{
	...
	for (int i = 0; i < g_ts_radio_cmd_num; i ++) {
		...
		gary_simple_uart_put_string((uint8_t*)cmd, cmd_length);
	}
			...
			switch (cmdIndex) {
				...
				case 3:
					...
					if (power >= 4) {
						...
						g_ts_radio_dtm_power_level = UESB_TX_POWER_0DBM;
					} else if (power >= -4) {
						g_ts_radio_dtm_power_level = UESB_TX_POWER_NEG4DBM;
					} else if (power >= -8) {
						g_ts_radio_dtm_power_level = UESB_TX_POWER_NEG8DBM;
					} else if (power >= -12) {
						g_ts_radio_dtm_power_level = UESB_TX_POWER_NEG12DBM;
					} else if (power >= -16) {
						g_ts_radio_dtm_power_level = UESB_TX_POWER_NEG16DBM;
					} else if (power >= -20) {
						g_ts_radio_dtm_power_level = UESB_TX_POWER_NEG20DBM;
					} else if (power >= -30) {
						g_ts_radio_dtm_power_level = UESB_TX_POWER_NEG30DBM;
					}
					break;
				default:
					break;
...
void print_ts_radio_dtm_cmd(int cmdIndex)
{
	...
	gary_simple_uart_put_string((uint8_t*)userStr, strlen(userStr));
}
