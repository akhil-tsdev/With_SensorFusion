...
#endif
#define SPI_COMM_LENGTH MAX(sizeof(nordic_to_hub_dtm_t),sizeof(hub_to_nordic_dtm_t))
	

nordic_to_hub_dtm_t				spi_tx;
hub_to_nordic_dtm_t 			spi_rx;
...
static bool packet_valid(hub_to_nordic_dtm_t packet) {
...
static void process_command(void) {
	...
	if ((spi_rx.cmd_type != dtm_cmd_type) || (spi_rx.dtm_tx_freq != dtm_freq) || 
			(spi_rx.dtm_tx_pkt_len != dtm_pkt_len) || (spi_rx.dtm_pkt_type !=  dtm_pkt_type) ||
			(spi_rx.dtm_power != dtm_power) || (spi_rx.tx_interval != dtm_tx_interval_ms)) {

		dtm_new_cmd = true;					
		dtm_cmd_type = spi_rx.cmd_type;
		dtm_freq = spi_rx.dtm_tx_freq;
		dtm_pkt_len = spi_rx.dtm_tx_pkt_len;		/* Max is UESB_CORE_MAX_PAYLOAD_LENGTH */
		dtm_power = spi_rx.dtm_power;
		dtm_pkt_type = spi_rx.dtm_pkt_type;
		dtm_tx_interval_ms = spi_rx.tx_interval; 
	}
}

static void prep_tx_crc16() {
