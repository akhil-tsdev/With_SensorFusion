...
static void on_radio_disabled_esb_dpl_rx(void)
{
...
#else
	/* For turingsense, we don't want to send ACK */
	...
	if(send_ack)
	{
		...
		if((m_uesb_mainstate == UESB_STATE_PRX_SEND_ACK_PAYLOAD) && (m_tx_fifo.count > 0))
			...
			// EDIT BY cwajh: There is a degenerate case: if the sender reinitialized twice in a row, and CRC
			// is turned off, such a packet will be detected as a "duplicate/retransmit" no matter what. WE
			// know we are not retransmitting, though, because we don't use that feature! So here's a hack:
			// if CRCs are turned off, always set the "last CRC" as some invalid pattern.
			m_last_rx_packet_crc = (m_config_local.crc == UESB_CRC_OFF) ? 0xFFFFFFFF : NRF_RADIO->RXCRC;
		} else {
			static int generate_code_here = 0;
			generate_code_here++;
		}
...
uint32_t uesb_force_reinit(uesb_config_t *parameters) {
	switch(m_uesb_mainstate) {
		case UESB_STATE_PTX_TX:
		case UESB_STATE_PTX_TX_ACK:
    case UESB_STATE_PTX_RX_ACK:
			// No way that I know of to cancel a TX state. Jam it.
			m_uesb_mainstate = UESB_STATE_IDLE;
			break;
    case UESB_STATE_PRX:
		case UESB_STATE_PRX_SEND_ACK:
    case UESB_STATE_PRX_SEND_ACK_PAYLOAD:
			uesb_stop_rx();
		  break;
		case UESB_STATE_UNINITIALIZED:
    case UESB_STATE_IDLE:
			// Continue.
			break;
	}
	uesb_disable();
	return uesb_init(parameters);
}

uesb_mainstate_t uesb_get_state_debug(void)
{ return m_uesb_mainstate;}

uint8_t uesb_get_rf_channel_debug(void)
