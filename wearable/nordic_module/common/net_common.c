...
volatile bool rf_packet_received = false;
...
#if HUB_SNIFFER
		uint32_t sender = (payload.data[3] << 24) | (payload.data[2] << 16) | (payload.data[1] << 8) | payload.data[0];
		rx_callback(payload.length, sender, payload.rssi);
#else 
...
#else /* SAT_NORDIC */
			if (payload.length == sizeof (sth_packet_t)) {
				rx_callback(&(payload.data[0]), sth_packet);
			} else if (payload.length == sizeof (hts_packet_t)) {
				rx_callback(&(payload.data[0]), hts_packet);
			}
#endif
		}
#endif /* HUB_SNIFFER */
