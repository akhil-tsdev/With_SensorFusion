...
# TODO: Will this break anything?
    ...
    def read(self):
        p = None
        while True:
            count = self.s.inWaiting()
            if count == 0:
                break
            self.buf = self.buf + self.s.read(count)
            i_dollar = self.buf.find('$')
            if i_dollar < 0:
                self.buf = ''
                continue
            self.buf = self.buf [i_dollar:]
            i_semi = self.buf.find(';')
            if i_semi < 0:
                i_dollar = self.buf.rfind('$');
                if i_dollar >= 0:
                    self.buf = self.buf [i_dollar:]
                continue
            rs = self.buf[:i_semi]
            self.buf = self.buf[i_semi:]
            #print "packet:", rs
            i_dollar = rs.rfind('$')
            rs = rs[i_dollar:]
            pkt_code = rs[1:3]
            if pkt_code == '1,':
                    p = quat_packet(rs[3:-1])
