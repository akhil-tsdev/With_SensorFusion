...
{ 
    A_STATUS error = ENET_mediactl (handle,ENET_MEDIACTL_VENDOR_SPECIFIC,inout_param);
...
A_STATUS ath_set_mode(A_UINT32 device,char* mode)
{
    ...
    else
    {
        ...
        error = ENET_mediactl (handle,ENET_SET_MEDIACTL_MODE,&inout_param);
        if (ENET_OK != error)
        {
            return error;
        }
...
A_STATUS ath_send_wep_param(ENET_WEPKEYS* wep_param){
  return(ENET_mediactl (handle,ENET_SET_MEDIACTL_ENCODE,wep_param));
}
	...
	do{
		...
		{	
			...
			if(pNetparams->sec_type == ENET_MEDIACTL_SECURITY_TYPE_WPA2){
		        ...
		        if(A_OK != ENET_mediactl (handle,ENET_MEDIACTL_VENDOR_SPECIFIC,&ath_param)){
		        	break;
		        }
		        strcpy(ver,"wpa2");
...
{	
   ...
   /*Check if driver is loaded*/
    ...
    error = ENET_mediactl (handle,ENET_GET_MEDIACTL_POWER,&inout_param);
    if (A_OK != error)
    {
        return error;
    }
...
A_INT32 wmi_set_scan(A_UINT32 dev_num)
{
    ...
    if (dev_num < IPCFG_DEVICE_COUNT)
    {
        ...
        error = ENET_mediactl (handle,ENET_SET_MEDIACTL_SCAN,NULL);
        if (A_OK != error)
        {
            return error;
        }

        error = ENET_mediactl (handle,ENET_GET_MEDIACTL_SCAN,&param);
        if (A_OK != error)
        {
            return error;
        }
...
void SystemReset()
{
	...
	ath_param.length = sizeof(value);
        ...
        while((error = ENET_mediactl (handle,ENET_MEDIACTL_IS_INITIALIZED,&dev_status))!=A_OK)
        {
...
A_INT32 wmi_ping(A_INT32 argc, char* argv[] ){
    ...
    {
        error =t_ping((void*) handle,hostaddr,size);
...
A_INT32 wmi_ping6(A_INT32 argc, char* argv[] ){
    ...
    for (index = 0; index < count; index++)
    {
        error =t_ping6((void*) handle,ip6addr,size);
