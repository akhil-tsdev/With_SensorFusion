...
import serial, sys, time, string, pygame, os
import struct
from ponycube import *

class eMPL_packet_reader:
    def __init__(self, port, quat_delegates=None, debug_delegate=None, data_delegate=None ):
...
# TODO: Will this break anything?
        ...
        if quat_delegates and len(quat_delegates):
            self.quat_delegates = quat_delegates
        else:
            self.quat_delegates = [empty_packet_delegate()]
        ...
        while self.s.inWaiting() >= NUM_BYTES:
            ...
            if ord(rs[0]) == ord('$'):
                ...
                if pkt_code == 1:
                    ...
                    p = quat_packet(rs)
                    if(p.is_valid()):
                        self.quat_delegates[ord(rs[2])%len(self.quat_delegates)].dispatch(p) 
                elif pkt_code == 3:
                    d = data_packet(rs)
...
class cube_packet_viewer (packet_delegate):
    def __init__(self,offset=(0,0)):
        self.screen = Screen(x=220*4,y=220*4,origin_offset=offset,scale=1.4)#480,400,scale=1.5)
...
class quat_packet (object):
    def __init__(self, l):
        self.l = l
        self.q0 = struct.unpack("<f",l[3:7])[0]
        self.q1 = struct.unpack("<f",l[7:11])[0]
        self.q2 = struct.unpack("<f",l[11:15])[0]
        self.q3 = struct.unpack("<f",l[15:19])[0]
    ...
    def is_valid(self):
        return all((q <= 1 and q >= -1) for q in [self.q0,self.q1,self.q2,self.q3])
...
if __name__ == "__main__":
    ...
    pygame.init()
    viewers = [cube_packet_viewer((-330,-440)),cube_packet_viewer((-110,-440)),cube_packet_viewer((110,-440)),cube_packet_viewer((330,-440)),
                   cube_packet_viewer((-330,-220)),cube_packet_viewer((-110,-220)),cube_packet_viewer((110,-220)),cube_packet_viewer((330,-220)),
	           cube_packet_viewer((-330,0)),cube_packet_viewer((-110,0)),cube_packet_viewer((110,0)),cube_packet_viewer((330,0)),
	           cube_packet_viewer((-330,220)),cube_packet_viewer((-110,220)),cube_packet_viewer((110,220)),cube_packet_viewer((330,220))
	           ]
