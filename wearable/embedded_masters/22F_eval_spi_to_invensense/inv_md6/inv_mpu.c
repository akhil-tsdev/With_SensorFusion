/*
 $License:
    ...
    Modified by Embedded Masters to separate platform interface into inv_platform.[ch],
    to never use bypass mode (define NO_BYPASS_MODE), and to always set MULT_MST_EN bit.
 $
 */
/**
...
/* never use bypass mode because we have aux I2C hooked up directly */
#define NO_BYPASS_MODE
...
static int get_st_biases(long *gyro, long *accel, unsigned char hw_test)
{
    ...
    data[0] = 0x80;  // Embedded Masters: added MUST_MST_EN bit
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
    data[0] = 0x00;  // Embedded Masters: set data back to zero
...
static int setup_compass(void)
{
    ...
    data[0] = 0xc0;  // Embedded Masters: added MUST_MST_EN bit
    if (i2c_write(st.hw->addr, st.reg->i2c_mst, 1, data))
        return -1;
