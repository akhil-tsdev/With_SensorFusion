/*
 * mpu9250.c 
 * -- Interfacing with the Nordic
 *  Created on: Dec 14, 2017
 *      Author: eric, Chitrang
 */
...
uint8_t cs_type[MAX_SENSORS_IN_CS] = {0};

extern const nrf_drv_spi_t spi;
extern void set_mpu9250_sensor_num(uint8_t num);

/*
 *  CS indicates out of 7 sensors which sensor has been selected.
 *  The byte to send to select the sensor over the UART has only three most significant bits most relevent. other five
 *  bits are dont cares.
 *
 */
void mpu9250_set_csn(uint8_t cs) {
...
void write_reg(uint8_t reg_addr, uint8_t data)
{
	...
	mts_buffer[1] = data;
	mpu9250_set_csn(mpu9250_num);
	nrf_delay_us(80); // CT --  This delay needed is around 15us not 1 ms.
	APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, mts_buffer, 2, stm_buffer, 2));
	nrf_delay_us(80);// CT --  This delay needed is around 15us not 1 ms.
	mpu9250_set_csn(0);
...
void read_regs(uint8_t reg_addr, size_t count, uint8_t *data)
{
	...
	mpu9250_set_csn(mpu9250_num);
	nrf_delay_us(80);               // CT --  This delay needed is around 15us not 1 ms.
	APP_ERROR_CHECK(nrf_drv_spi_transfer(&spi, mts_buffer, count + 1, stm_buffer, count + 1));
	nrf_delay_us(80);               //  CT --  This delay needed is around 15us not 1 ms.
	mpu9250_set_csn(0);
...
static void write_mag_reg(uint8_t reg_addr, uint8_t data)
{
	...
	nrf_delay_ms(30);			// delay for write to complete, should happen every 10 ms

	write_reg(MPU9250_REG_I2C_SLV4_CTRL, (MAG_SUBSAMPLE-1));      // disable

	nrf_delay_ms(30);          // delay for write to stop happening
}
...
static uint8_t read_mag_reg(uint8_t reg_addr)
{
	...
	nrf_delay_ms(30);			// delay for reaad to complete, should happen every 10 ms

	read_regs(MPU9250_REG_I2C_SLV4_DI, 1, & data);

	write_reg(MPU9250_REG_I2C_SLV4_CTRL, (MAG_SUBSAMPLE-1));      // disable

	nrf_delay_ms(30);          // delay for read to stop happening
...
bool mpu9250_detect_sensor(uint8_t cs, uint8_t* whoami) {
	...
	*whoami = 0;
	...
	mpu9250_set_csn(0); // To de-select the CS pin. Just for the clean start.
	//mpu9250_set_csn(cs);
	

	uint8_t data[MPU9250_REG_COUNT];

	read_regs(MPU9250_REG_WHO_AM_I, 1, data);
	*whoami = data[0];
	//mpu9250_set_csn(0); // To de-select the CS pin.
...
bool mpu9250_data_avail(void)
{
	...
	read_regs(MPU9250_REG_INT_STATUS, 1, &data);
				  ...
				  sensor_data_t *sensor_data)
{
	...
	sensor_data->timestamp =  0 ;//OSA_TimeGetMsec();
		               ...
		               mpu9250_average_data_t *selftest)
{
	...
	nrf_delay_ms(20); // wait 20ms for oscillations from enabling self-test to stabilize
	...
	nrf_delay_ms(20); // wait 20ms for oscillations from disabling self-test to stabilize
