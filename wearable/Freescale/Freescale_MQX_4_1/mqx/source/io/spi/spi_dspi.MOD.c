   ...
   )
{
...
#ifndef TURINGSENSE
    /* TURINGSENSE: cwati Let's not do this dummy thing */
    ...
    /* Set CS signals */
    /* TURINGSENSE: cwati WOW this CS set is totally blind about active high or active low */
    ...
    cs = (dspi_ptr->MCR & SPI_MCR_PCSIS_MASK);
    if (params->CS_active_low) {
    	cs |= params->CS;
    } else { /* CS active high */
    	cs &= ~(unsigned)(params->CS);
    }

    dspi_ptr->MCR = (dspi_ptr->MCR & ~SPI_MCR_PCSIS_MASK) | SPI_MCR_PCSIS(cs);
#endif /* TURINGSENSE */
        ...
        while (rx_len)
        {
            if (tx_len) {
                if ((dspi_ptr->SR & DSPI_SR_TFFF_MASK) && ((rx_len-tx_len)<DSPI_FIFO_DEPTH))
                {
                    if (txbuf)
                        data = *txbuf++;
                    //cwati experiment, adding PUSHR info for CONT, CTAS, PCS, EOQ, CTCNT
//                    if (tx_len > 1) {
//                    	/* CONT, PCS=2, CTCNT */
//						data = data | 0x84020000;
//                    } else if (tx_len == 1) {
//                    	/* PCS=2, EOQ */
//						data = data | 0x08020000;
//                    }
                    dspi_ptr->PUSHR = DSPI_PUSHR_TXDATA(data);
                    dspi_ptr->SR = DSPI_SR_TFFF_MASK;
                    tx_len--;
...
{
	/* TURINGSENSE: cwati on MK22F51212, this should be done automatically
	 * when we push to SPI PUSHR register.  Writing to MCR PCSIS is messing up
	 * the CS inactive state selection.
	 */
#ifndef TURINGSENSE
    DSPI_INFO_STRUCT_PTR               dspi_info_ptr = (DSPI_INFO_STRUCT_PTR)io_info_ptr;
    VDSPI_REG_STRUCT_PTR               dspi_ptr = dspi_info_ptr->DSPI_PTR;
