...
A_INT32 Api_recvfrom(A_VOID *pCxt, A_UINT32 handle, void** buffer, A_UINT32 length, A_UINT32 flags, A_VOID* name, A_UINT32* socklength)
{
    ...
    else
    {
		while((a_netbuf_ptr = A_NETBUF_DEQUEUE(&(pcustctxt->rxqueue))) == NULL)
		{
#if NON_BLOCKING_RX
			/* Turingsense
			 * For NON_BLOCKING_RX, we need better return value to know if RX
			 * is not receiving data, or if peer socket has closed connection.
			 */
			...
			/* If peer closed connection, return A_SOCK_INVALID */
...
#if TURINGSENSE
            if(CUSTOM_BLOCK(pCxt, ath_sock_context[index], TS_RECV_BLOCK_TIMEOUT, RX_DIRECTION) != A_OK)
#else
