...
err_t add_rx_from_nordic(nordic_to_hub_t rx_data) {
	...
	for (cnt = 0; cnt < MAX_SENSOR_RECORDS_PER_PACKET; cnt++) {
		...
		if(rx_data.record[cnt].sensor_idx == INVALID_SAT_IDX) {
			continue;
		}
#if PRINT_DEBUG
	...
	/* Add data to cbuf */
        ...
        for(int i = 0;i<MAX_SENSOR_RECORDS_PER_PACKET;i++){//cf: packet counter
          ...
          if(tmp==CPI_QUATERNION16_COUNTER_COMMAND){
            test=1;
            for(int cntSat = 0;cntSat<MAX_SENSORS;cntSat++){//cf: packet counter
              if(cntSat==rx_from_nordic.record[i].sensor_idx){
                  if(communicationTestCounter_lastSAT22F[cntSat]<rx_from_nordic.record[i].data.accel_x){
                    communicationTestCounter_lastSAT22F[cntSat]=rx_from_nordic.record[i].data.accel_x;
                    communicationTestCounter[cntSat]++;
                  }else if(rx_from_nordic.record[i].data.accel_x <= 0){
                     communicationTestCounter[i] = 0;
                     communicationTestCounter_lastSAT22F[i]=0;
                  }else if(communicationTestCounter_lastSAT22F[cntSat]>rx_from_nordic.record[i].data.accel_x){
                    communicationTestCounter_lastSAT22F[cntSat] = rx_from_nordic.record[i].data.accel_x;
                  }
#if ENABLE_MAG
                  rx_from_nordic.record[i].data.mag_x = (int16_t)(communicationTestCounter[cntSat]/CPI_QUATERNION16_COUNTER_MAXVALU);
                  rx_from_nordic.record[i].data.mag_y = (int16_t)(communicationTestCounter[cntSat]%CPI_QUATERNION16_COUNTER_MAXVALU);
#else
                  rx_from_nordic.record[i].data.quat_x = (int16_t)(communicationTestCounter[cntSat]%CPI_QUATERNION16_COUNTER_MAXVALU);
#endif
               }
            }
            //rx_from_nordic.record[i].data.mag_z = cur_sat_ids[1];
          }
         }
         if(!test && communicationTestCounter[0]>0)
         {
            for(int cntSat = 0;cntSat<MAX_SENSORS;cntSat++){
              communicationTestCounter[cntSat] = 0;
              communicationTestCounter_lastSAT22F[cntSat]=0;
            } 
         }
        
        //for(int i = 0;i<MAX_SENSOR_RECORDS_PER_PACKET;i++)//cf: packet counter
         // if(rx_from_nordic.record[i].data.quat_w==CPI_QUATERNION16_COUNTER_COMMAND)
          //  rx_from_nordic.record[i].data.mag_z = rx_from_nordic.record[i].sat_idx;
#if FAST_DRAIN
