...
void sendSatIdToNordic(uint32_t pos, uint32_t satIDToSend, uint32_t curSatNum)
{
...
void sendHubIdToNordic(void)
{
        ...
        /* Without delays at all these commands seem to not reach Nordic properly */
        app_time_delay(200);
...
void hub_main_loop(uint32_t dummy) {
	...
	ts_turn_on_LED(greenLed);
        ...
        // Read Hub Id 
        cur_hub_id = readHubIdFromFlash();
        sprintf(&ssid_name[0], "HUB%u_%s_%s_%s.%s.%s.%s_ch%u",cur_hub_id,STR(MAJOR_VERSION),STR(MINOR_VERSION),STR(REVIS_VERSION_SAT22F),\
          STR(REVIS_VERSION_SATNRD),STR(REVIS_VERSION_HUB22F),STR(REVIS_VERSION_HUBNRD),WIFI_START_CHANNEL);
        ...
        /* Set Hub SSID */
        sprintf(&ssid_name[0], "PIVOT3_0_POC_%u",cur_hub_id);
