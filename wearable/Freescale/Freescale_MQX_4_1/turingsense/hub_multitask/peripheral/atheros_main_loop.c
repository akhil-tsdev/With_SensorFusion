...
uint32_t communicationTestCounter_fromNordic[MAX_SENSORS]={0}; //CF: communication analysis
uint32_t communicationTestCounter_toClient[MAX_SENSORS]={0}; //CF: communication analysis
uint32_t communicationTestCounter_1[MAX_SENSORS]={0}; //CF: communication analysis
uint32_t communicationTestCounter_2[MAX_SENSORS]={0}; //CF: communication analysis
uint8_t timeZone = 5; //CF for testing
...
/* For 100Hz -> group per 10ms. */
 uint32_t		timestamp_group = 1000 / SAT_SENDING_FREQ; //cwati ssc 200Hz
...
static A_INT32 hub_send_to_wifi(hub_to_cloud_t *tmp_cbuf_to_cloud) {
	...
	for(int i = 0;i<MAX_SENSORS;i++){//cf: packet counter
		if(tmp_cbuf_to_cloud->data[i].record.quat_w==CPI_QUATERNION16_COUNTER_COMMAND){
                  if(communicationTestCounter_2[i]<tmp_cbuf_to_cloud->data[i].record.accel_x){
                        communicationTestCounter_2[i]=tmp_cbuf_to_cloud->data[i].record.accel_x;
			communicationTestCounter_toClient[i]++;
                  }else if(tmp_cbuf_to_cloud->data[i].record.accel_x <= 0){
//                     communicationTestCounter_toClient[i] = 0;
//                     communicationTestCounter_2[i]=0;
                  }else if(communicationTestCounter_2[i]>(tmp_cbuf_to_cloud->data[i].record.accel_x)){
                    communicationTestCounter_2[i] = tmp_cbuf_to_cloud->data[i].record.accel_x;
                  }
                  
//		  tmp_cbuf_to_cloud->data[i].record.quat_x = (int16_t)(communicationTestCounter[i]/CPI_QUATERNION16_COUNTER_MAXVALU);
//		  tmp_cbuf_to_cloud->data[i].record.quat_z = (int16_t)(communicationTestCounter[i]%CPI_QUATERNION16_COUNTER_MAXVALU);
		  tmp_cbuf_to_cloud->data[i].record.quat_y = communicationTestCounter_fromNordic[i];
		  tmp_cbuf_to_cloud->data[i].record.quat_z = communicationTestCounter_toClient[i];                  
                }
	}
...
static A_INT32 hub_send_to_wifi_batch() {
	...
	for ( /* Condition is set above */ ; sensor_idx < cur_num_of_sensors; sensor_idx++) {
          ...
          } else if (abs(cbuf[sensor_idx].A[cbuf[sensor_idx].head].timestamp - lowest_ts) <= (timeZone*timestamp_group) || !timeZone) {
...
//                  cloud_data.payload.satellite_ids[SAT_COMMSTRUCT_VALY2],cloud_data.payload.satellite_ids[SAT_COMMSTRUCT_VALZ3]);
	  ...
	  if(tmp>0 && tmp<256)
        ...
        case SAT_COMM_TESTCOMSTC:
            	for(int i = 0;i<MAX_SENSORS;i++)//cf: packet counter
		{
                  communicationTestCounter_1[i] = 0;
                  communicationTestCounter_2[i] = 0;
                  communicationTestCounter_fromNordic[i] = 0;
                  communicationTestCounter_toClient[i] = 0;
                }
            onlyTest(false);
            break;
        case SAT_COMM_SENDACKFRM:
          onlyTest(false);
          ack_to_client = 1;
          break;
        case SAT_COMM_HUBCONFIG1:
          timeZone = cloud_data.payload.satellite_ids[SAT_COMMSTRUCT_VALX1];
	default:
