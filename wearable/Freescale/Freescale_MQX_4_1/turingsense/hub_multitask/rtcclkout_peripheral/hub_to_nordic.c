...
err_t add_rx_per_sensor_record(sat_sensor_record_t rx_from_nordic) {
	...
	CB_TYPE		new_cbuf_entry;
        static uint32_t discarded[MAX_SENSORS] = {0}; //debug
...
uint32_t talk_to_nordic(nordic_to_hub_t *data)
{
	 ...
	 */
	if (hub_can_send(current_state) && hub_active(current_state)) {
	...
	/* Add data to cbuf */
        ...
        for(int i = 0;i<MAX_SENSOR_RECORDS_PER_PACKET;i++)//cf: packet counter
          if(rx_from_nordic.record[i].data.quat_w/CPI_QUATERNION16_BIAS_MULTIPLIER==CPI_QUATERNION16_COUNTER_COMMAND){
            test=1;
            for(int cntSat = 0;cntSat<MAX_SENSORS;cntSat++){//cf: packet counter
              if(cntSat==rx_from_nordic.record[i].sat_idx){
                  if(communicationTestCounter_lastSAT22F[cntSat]<rx_from_nordic.record[i].data.accel_y){
                    communicationTestCounter_lastSAT22F[cntSat]=rx_from_nordic.record[i].data.accel_y;
