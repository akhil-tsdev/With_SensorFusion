...
void computeAverageVector(float meanACC[3], float meanGYR[3], float meanMAG[3], uint32_t* magvalid, uint32_t* totalread)
{
	int c = 0;
	int N = 200;
	int offset = 50;
	...
	while (c < N+offset)
	{
		...
		/* Ready every 10ms for 100Hz rate */
		OSA_TimeDelay(9);

		get_raw_data_sample_basic(&raw_data);
//		raw_data.valid_sensors = 0;
//
//		while (! ((raw_data.valid_sensors & SENSOR_ACCEL) && (raw_data.valid_sensors & SENSOR_GYRO) &&
...
//		raw_data.valid_sensors = 0;
//
//		static uint16_t cnt_no_data = 0;
//
//		while (!(raw_data.valid_sensors & SENSOR_MAG)) {
//			OSA_TimeDelay(10);
//			get_raw_data_sample_basic(&raw_data);
//			cnt_no_data++;
//		}
