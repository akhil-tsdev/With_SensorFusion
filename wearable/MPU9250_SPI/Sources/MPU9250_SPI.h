/* Turingsense 2014
 * cwati - And His song will be with me in the night,
...
//#include "fsl_os_abstraction.h" 	/* Delay */
#include <stdint.h>
...
/* 9250 Pins */
#define MPU9250_GND 		18
#define MPU9250_VDD 		13
#define MPU9250_VDDIO 		 8
#define MPU9250_FSYNC 		11
#define MPU9250_SPI_CS 		(*((volatile unsigned long *) 0x22))

/* 9250 SPI */
#define MPU9250_SPI_SCLK 	(*((volatile unsigned long *) 0x23))
#define MPU9250_SPI_MOSI 	(*((volatile unsigned long *) 0x24))
#define MPU9250_SPI_MISO 	(*((volatile unsigned long *) 0x09))
#define MPU9250_SPI_IRQ 	(*((volatile unsigned long *) 0x12))

#define MPU9250_SPI_TXBUF    (*((volatile unsigned long *) 0x0))
#define MPU9250_SPI_RXBUF    (*((volatile unsigned long *) 0x1))  //cwati to do

#define MPU_InitRegNum 17

typedef enum mpu9250_err_ {
    mpu9250_ok,
    mpu9250_err_unknown,
} mpu9250_err;

typedef struct mpu9250_spi_ {
    /*
...
/*
 * Initialization function.  Caller must call this to initialize SPI.
 *
 * Give sample rate divider (0 to 255) and low pass filter value.
 ...
 *
 */
mpu9250_err mpu9250_spi_init(int ,int);

/*
 * Enabling/disabling MPU9250 communication bus.
 * Start transmission by setting CS low.
 */
void mpu9250_spi_select(void);

/*
 * Enabling/disabling MPU9250 communication bus.
 * Stop transmission by setting CS high.
 */
void mpu9250_spi_deselect(void);

/*
 * Writing SPI
 *
 * Reference: CWATI to delete
int msp430_spi_write(uint8_t sel, uint8_t reg_addr,
...
/*
 * Send data to SPI
 * CWATI MUST IMPLEMENT THISSSSS
 * Return value is what's read from the TXBUF register
 */
uint32_t mpu9250_spi_send(uint8_t WriteAddr);

/*
 * Reading SPI one byte
 */
uint32_t mpu9250_spi_ReadReg( uint8_t WriteAddr, uint8_t WriteData );

/*
 * Reading SPI multiple byte
 */
 ...
 *
 * Usage: Call this after initialization to set accelerometer's range.
 * Returns: Range set
 */
...
/*
 * Function to check if SPI is working correctly.
 * It checks the I2C address of MPU9250 which should be 104 when in SPI mode
 * Returns the I2C address (104)
 */
uint32_t mpu9250_whoami(void);
