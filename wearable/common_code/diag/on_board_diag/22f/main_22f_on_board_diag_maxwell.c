...
uint8_t get_uart_num(uint8_t this_sensor_num) {
...
int main(void) {
    ...
    uint16_t				user_input;
    uint8_t 				uart_num = 0xff;
	uint8_t 				whoami;
	char 					yesno;
	bool 					spi_integrity_result[MAX_SENSORS_IN_CS] = {0};
	bool					self_test_result[MAX_SENSORS_IN_CS] = {0};
	uint8_t					sensor_num;
#if DO_SELFTEST
...
//	debug_printf("GOT HERE \n\r");
	...
	while (true) {
		debug_printf("\n\r***********************************************\r");
		debug_printf("M| DTM TEST ENVIRONMENT version %s.  UART & STM.  Choose an optionX:\r",VERSION_RELEASE);
		debug_printf("M|  0 - Options\r");
		debug_printf("M|  1 - Set tolerances\r");
		debug_printf("M|  2 - (N/A) Execute test 1.A: debug SPI\r");
		...
		debug_printf("M|  6 - Generate SAT_ID\r");
		debug_printf("M|  7 - (N/A) RF test\r");
		debug_printf("M|  8 - (N/A) RF reset parameters\r");
		debug_printf("M|  9- (N/A) Magnetometer Output Test\r");
		debug_printf("M|  10- Garment Test\r");
		debug_printf("E|\r\n");
		...
		if(user_input == 0){
			...
			uart_num = invalid_maxwell_num;
			while ((uart_num > MAX_SENSORS_IN_CS) || (uart_num == invalid_maxwell_num)) {
				debug_printf("\r\nWhich sensor (uart number)? [1-%u]: ", MAX_SENSORS_IN_CS);
				debug_scanf("%u", &uart_num);
			}

			mpu9250_init_cs_gpio();
			mpu9250_init_all_pins(); //cwati all except for CS

			debug_printf("\r\nEnabling sensor (1-7) %u...", uart_num);

			lsm6dsl_detect_sensor(uart_num, &whoami);
			if (whoami == LSM6DSL_WHOAMI) {
				debug_printf("\r\nGot STM LSM6DSL in uart number %u", uart_num);
				debug_printf("\r\nRunning SPI Sensor Register Write Test..");
				lsm6dsl_spi_test();
			} else {
				debug_printf("\r\nCan't reach sensor in uart number %u", uart_num);
			}
		...
		} else if (user_input == 4)
		{
			...
			if (whoami == LSM6DSL_WHOAMI) {
				debug_printf("\r\nGot STM LSM6DSL in uart number %u", uart_num);
				debug_printf("\r\nDo you want to run Self-Test? (y/n):");
				debug_scanf("%c", &yesno);

				if (yesno == 'y' || yesno == 'Y') {
					set_uart_num(0, uart_num);
					sat_enable_sensor(0, uart_num);
					lsm6dsl_init_regs();

					retbool = tsSelftestIndividual(0);
					if (retbool == true) {
						debug_printf("\r\nSelf-test for sensor (UART num) %d: PASS", uart_num);
					} else {
						debug_printf("\r\nSelf-test for sensor (UART num) %d: FAIL", uart_num);
					}
				}
			} else {
				debug_printf("\r\nCan't reach sensor in uart number %u", uart_num);
...
//				}
//			} else {
//				debug_printf("\r\nSensor in CS %u is not properly read!", uart_num);
//			}
		}
		else if (user_input == 5)
			debug_printf("[SATDTM=" VERSION_RELEASE "]\r\n");
...
//			checkMagOutput(parameters);
		...
		}  else if (user_input == 10)
		{
			...
			for (uart_num = 1; uart_num < (MAX_SENSORS_IN_CS + 1); uart_num++) {
				spi_integrity_result[valid_num_sensors_in_cs] = false;
				self_test_result[valid_num_sensors_in_cs] = false;

				lsm6dsl_detect_sensor(uart_num, &whoami);
				if (whoami == LSM6DSL_WHOAMI) {
//					if (parameters.log_enableDetail > 1) {
						debug_printf("\r\nSensor %u uart %u: Found\n", valid_num_sensors_in_cs, uart_num);
//					}
					set_uart_num(valid_num_sensors_in_cs, uart_num);

					sat_enable_sensor(valid_num_sensors_in_cs, uart_num);
					...
					spi_integrity_result[valid_num_sensors_in_cs] = lsm6dsl_spi_test();

					debug_printf("\r\nRunning Self-test %u uart %u...DO NOT move the sensor!\n", valid_num_sensors_in_cs, uart_num);
					self_test_result[valid_num_sensors_in_cs] = tsSelftestIndividual(valid_num_sensors_in_cs);

					valid_num_sensors_in_cs++;
				} else {
					debug_printf("\r\nSensor %u uart %u: Not Found\n", valid_num_sensors_in_cs, uart_num);
				}
			}

			/* Print results from Individual sensor test */
			debug_printf("\n\rSENSOR(Uart Num)|SPI     |SelfTest|PASS/FAIL");
			debug_printf("\n\r----------------------------------------------------------------------");

			for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
				debug_printf("\n\r%u(%u)\t\t|%s|%s|%s", sensor_num, get_uart_num(sensor_num),
						spi_integrity_result[sensor_num] ? "PASS    " : "FAIL    ",
						self_test_result[sensor_num] ? "PASS    " : "FAIL    ",
						spi_integrity_result[sensor_num] && self_test_result[sensor_num] ? \
								"PASS    " : "FAIL    ");
			}
