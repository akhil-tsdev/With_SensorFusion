...
bool positionsAcquired[6] = {false};
...
static sat_sensor_record_t sensorDataForInterpolation = { 0 };
 ...
 * it is not always the same with UART number.
 *
 * For example we can have up to 4 sensors with sensor_num 0 to 3.
 *
 * Sensor_num 0 is on UART 1
 * Sensor_num 1 is on UART 3
 * Sensor_num 2 is on UART 5
 * Sensor_num 3 is on UART 7
 *
 * As you can see, the UART number is not in order.
 *
 *
 */
uint8_t valid_num_sensors_in_cs = 0;
static uint8_t sensor_type[MAX_SENSORS_IN_CS] = {0};
static sensor_info_t sensor_info[MAX_SENSORS_IN_CS] = {0};

static uint8_t get_sensor_type(uint8_t this_sensor_num) {
	/* Sensor num is numbered 1-7, while the array
	 * is indexed 0-6.
	 */
	return (sensor_info[this_sensor_num].cs_type);
}

static void set_sensor_type(uint8_t this_sensor_num, uint8_t type) {
	sensor_info[this_sensor_num].cs_type = type;
}

static void set_uart_num(uint8_t this_sensor_num, uint8_t uart_num) {
	sensor_info[this_sensor_num].uart_num = uart_num;
}

static uint8_t get_uart_num(uint8_t this_sensor_num) {
	return (sensor_info[this_sensor_num].uart_num);
}
...
bool estimateGyroBias()
{
	...
	float biasFloat_X[MAX_SENSORS_IN_CS] = { 0.0f };
	float biasFloat_Y[MAX_SENSORS_IN_CS] = { 0.0f };
	float biasFloat_Z[MAX_SENSORS_IN_CS] = { 0.0f };
	...
	while (c < N) {
		...
		OSA_TimeDelay(5);
		for (uint8_t sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
				sat_enable_sensor(sensor_num, get_uart_num(sensor_num));

				get_raw_data_sample_basic(sensor_num, &raw_data);

				// compute the bias
				biasFloat_X[sensor_num] += ( (float) raw_data.gyro[0])*coeff;
				biasFloat_Y[sensor_num] += ( (float) raw_data.gyro[1])*coeff;
				biasFloat_Z[sensor_num] += ( (float) raw_data.gyro[2])*coeff;
		...
		for (uint8_t sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++)
		{
			gyrBiasInt16_X[sensor_num] = (int16_t) (biasFloat_X[sensor_num]/MPU9250_DEGPERSECPERCOUNT);
			gyrBiasInt16_Y[sensor_num] = (int16_t) (biasFloat_Y[sensor_num]/MPU9250_DEGPERSECPERCOUNT);
			gyrBiasInt16_Z[sensor_num] = (int16_t) (biasFloat_Z[sensor_num]/MPU9250_DEGPERSECPERCOUNT);
		}
...
bool waitForNextPosition(int p)
{
	...
	sat_enable_sensor(0, get_uart_num(0));
	...
	while (count < 200)
	{
		...
		count++;
		OSA_TimeDelay(5);
		get_raw_data_sample_basic(0, &raw_data);
...
bool changedPosition(float accX, float accY, float accZ, float lp[3])
{
	if ((lp[0] + lp[1] + lp[2]) == 0.0f)
		return true;

	// Compute angle
	float normAcc = sqrt(pow(accX,2)+pow(accY,2)+pow(accZ,2));
	float normLp = sqrt(pow(lp[0],2)+pow(lp[1],2)+pow(lp[2],2));
	...
	float dotP = accX*lp[0] + accY*lp[1] + accZ*lp[2];

	float diffAngle = acos(dotP)*180/PI;

	return (diffAngle > 45);
}

int getCurrentPosition(sensor_data_t raw_data, float accX, float accY, float accZ, float lastPosition[3])
{
	float M = 1.3;
	float m = 0.7;

	if (!detectMotion(&raw_data) & (changedPosition(accX,accY,accZ,lastPosition)) )
	{
		//if ((accX < M) & (accX > m))
		//	return X_UP;
		//if ((-accX < M) & (-accX > m))
...
bool waitForNextPosition2(float lastPosition[3])
{
	...
	while (count < 200)
	{
		...
		currPos = getCurrentPosition(raw_data,accX,accY,accZ,lastPosition);

		if (currPos == 1) //((currPos == 7) | (positionsAcquired[currPos] == true))
			count = 0;
	}

	positionsAcquired[currPos] = true;
...
bool estimateAccBias()
{
	...
	float data[MAX_SENSORS_IN_CS][3][6], J[6][6], iJ[6][6];
	...
	// reset acquired positions
	for (int i = 0; i < 6; i++)
		positionsAcquired[i] = false;

	for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {

		for (int i = 0; i < 6; i++)
			for (int j = 0; j < 3; j++)
				data[sensor_num][j][i] = 0.0f;
	...
	for (int i = 0; i < 6; i++)
	{
		/* Wait for next pos depends on 1st sensor */
		//stopCalAccCommand = !waitForNextPosition(i);
		OSA_TimeDelay(2000);
		stopCalAccCommand = !waitForNextPosition2(lastPosition);
		turnOnLED(turqoiseLed);
		...
		while (c < N)
		{
			...
			for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
				...
				// acquire the data
				data[sensor_num][0][i] += ( (float) raw_data.accel[0])*coeff;
				data[sensor_num][1][i] += ( (float) raw_data.accel[1])*coeff;
				data[sensor_num][2][i] += ( (float) raw_data.accel[2])*coeff;
		...
		// Update last position (based on sensor 0)
		lastPosition[0] = data[0][0][i];
		lastPosition[1] = data[0][1][i];
		lastPosition[2] = data[0][2][i];
	}
	...
	for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
		...
		{
			computeJacobianAccBias(data[sensor_num],x,J);
			invMatrixAccBias(J,iJ);
			computeResidualsAccBias(data[sensor_num], x, beta);
	...
	if ((estimateAccBias_run == false)||(cmd_sat == SAT_COMM_STOPCALACC))
	{
		...
		if(cmd_sat==SAT_COMM_TESTVALUE0)//test command 0
		{
			...
			for (uint8_t qq = 0; qq < valid_num_sensors_in_cs; qq++) {
				communicationTestCounter[qq][COMMUNICATIONCOUNTER_SPIP]=0;
				communicationTestCounter[qq][COMMUNICATIONCOUNTER_BUFF]=0;
			}
			for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
				sat_enable_sensor(sensor_num, get_uart_num(sensor_num));
			...
			fInitMagCalibration(&thisMagCal[mpu9250_num], &thisMagBuffer[0]);
...
//			//sensorDataForInterpolation = (sensor_record_t*)malloc(alternateSending_PERIOD*sizeof(sensor_record_t));
		...
		} else if (command_is_for_sat_nordic_only(cmd_sat)) {
			/* Do nothing.  Acknowledge that we got the command, but don't blink red */
			onlyTest(false);
		} else {
...
dspi_status_t to_nordic_data_source(uint8_t *sourceWord, uint32_t instance) {
	...
	if ((!hw_init_done) || (hub_err_status != hub_no_err)) {
		...
		invalidDataToSend.data.data.timestamp = INVALID_TIMESTAMP;
	...
	if (currSourceByte == SPI_COMM_LENGTH) {
		...
		if (errCode != E_OK) {
			...
			memcpy(&(dataToSend.data.data), &cbuf_data, sizeof(cbuf_data));
			if(communicationTestCounterInitialization < 0) {
				if (dataToSend.data.data.data.accel_x != 0) {
					communicationTestCounter[cbuf_data.sensor_idx][COMMUNICATIONCOUNTER_SPIP]++;
				}
			} else {
				communicationTestCounterInitialization--;
				communicationTestCounter[cbuf_data.sensor_idx][COMMUNICATIONCOUNTER_SPIP]=0;
				communicationTestCounter[cbuf_data.sensor_idx][COMMUNICATIONCOUNTER_BUFF]=0;
			}
		}
		...
		if (errCode != E_OK) {
			...
			dataToSend.data.data.timestamp = INVALID_TIMESTAMP;
		}
		currSourceByte = 0;
...
static bool get_raw_data_sample_basic(uint8_t sensor_num, sensor_data_t *raw_data) {
	...
	if(get_sensor_type(sensor_num) == MPU9250_VALID_WHO_AM_I)
...
static bool get_raw_data_sample(uint8_t sensor_num, sensor_data_t *raw_data, sat_sensor_record_t *sensor_record) {
	...
	bool ret = get_raw_data_sample_basic(sensor_num, raw_data);
	...
	if (ret)
		...
		{
			process_sensor_fusion(raw_data, sensor_record);
		}
		else
...
static sat_sensor_record_t addSetMeasurement(uint16_t mult, sat_sensor_record_t sensor_record){
...
//		sensor_record.accel_x = (sensor_record.accel_x + sensorDataForInterpolation.accel_x) / alternateSending_PERIOD;
//		sensor_record.accel_y = (sensor_record.accel_y + sensorDataForInterpolation.accel_y) / alternateSending_PERIOD;
//		sensor_record.accel_z = (sensor_record.accel_z + sensorDataForInterpolation.accel_z) / alternateSending_PERIOD;
//		sensor_record.gyro_x  = (sensor_record.gyro_x  + sensorDataForInterpolation.gyro_x ) / alternateSending_PERIOD;
//		sensor_record.gyro_y  = (sensor_record.gyro_y  + sensorDataForInterpolation.gyro_y ) / alternateSending_PERIOD;
//		sensor_record.gyro_z  = (sensor_record.gyro_z  + sensorDataForInterpolation.gyro_z ) / alternateSending_PERIOD;
//	#if ENABLE_MAG
//		sensor_record.mag_x   = (sensor_record.mag_x   + sensorDataForInterpolation.mag_x  ) / alternateSending_PERIOD;
//		sensor_record.mag_y   = (sensor_record.mag_y   + sensorDataForInterpolation.mag_y  ) / alternateSending_PERIOD;
//		sensor_record.mag_z   = (sensor_record.mag_z   + sensorDataForInterpolation.mag_z  ) / alternateSending_PERIOD;
//	#endif
//		sensor_record.quat_w  = (sensorDataForInterpolation.quat_w); //send previous value of QUAT
//		sensor_record.quat_x  = (sensorDataForInterpolation.quat_x);
//		sensor_record.quat_y  = (sensorDataForInterpolation.quat_y);
//		sensor_record.quat_z  = (sensorDataForInterpolation.quat_z);
//
//		if(globals.outputMode == OUTPUT_MODE_TIME){
//				// send timestamp in quaternion
//				uint16_t * timestramp = (int16_t*) &(sensor_record.timestamp);
//
//				sensor_record.quat_w = (int16_t)(CPI_QUATERNION16_BIAS_MULTIPLIER*CPI_QUATERNION16_TIMESTA_COMMAND);
//				sensor_record.quat_x = *timestramp;
//				sensor_record.quat_y = *(timestramp+1);
//				sensor_record.quat_z = 0;
//		}
//	}
//
...
//		sensorDataForInterpolation.accel_x = mult * (sensorDataForInterpolation.accel_x + sensor_record.accel_x);
//		sensorDataForInterpolation.accel_y = mult * (sensorDataForInterpolation.accel_y + sensor_record.accel_y);
//		sensorDataForInterpolation.accel_z = mult * (sensorDataForInterpolation.accel_z + sensor_record.accel_z);
//		sensorDataForInterpolation.gyro_x  = mult * (sensorDataForInterpolation.gyro_x  + sensor_record.gyro_x);
//		sensorDataForInterpolation.gyro_y  = mult * (sensorDataForInterpolation.gyro_y  + sensor_record.gyro_y);
//		sensorDataForInterpolation.gyro_z  = mult * (sensorDataForInterpolation.gyro_z  + sensor_record.gyro_z);
//#if ENABLE_MAG
//		sensorDataForInterpolation.mag_x   = mult * (sensorDataForInterpolation.mag_x   + sensor_record.mag_x);
//		sensorDataForInterpolation.mag_y   = mult * (sensorDataForInterpolation.mag_y   + sensor_record.mag_y);
//		sensorDataForInterpolation.mag_z   = mult * (sensorDataForInterpolation.mag_z   + sensor_record.mag_z);
//#endif
//		sensorDataForInterpolation.quat_w  =  (sensor_record.quat_w);
//		sensorDataForInterpolation.quat_x  =  (sensor_record.quat_x);
//		sensorDataForInterpolation.quat_y  =  (sensor_record.quat_y);
//		sensorDataForInterpolation.quat_z  =  (sensor_record.quat_z);
//	}
//
//		return sensor_record;
}
...
int main(void) {
	...
	sat_sensor_record_t sensor_data[MAX_SENSORS_IN_CS];// Timestamp + sensor data (raw + sensor-fusion output)
	uint8_t	 sensor_data_valid;
	uint32_t	sensor_data_ts;
	uint32_t current_ts10, time_now;
	uint8_t	whoami = 0xff;
	bool is_sleeping = false, retbool;
	uint8_t sensor_num, uart_num;
	bool has_data;
...
//		printf("error initializing OSA\r\n");
	...
	configure_uart_pins(REV_F_UART_INSTANCE);

    // Initialize the uart module with base address and config structure
    UART_DRV_Init(REV_F_UART_INSTANCE, &uartState, &uartConfig);
//
//    if (init_spi_to_sensor() == false) {
//		printf("error initializing SPI to sensors\r\n");
//		blinkLEDerror(redLed, 1);
//		hub_err_status = hub_hw_err1;
//    }
...
//	printf("\r\n(Freescale's Open Source Sensor Fusion is used instead of Invensense's Motion Driver 6.1)\r\n");
	...
	/* UART numbered 1 through 7 */
	for (uart_num = 1; uart_num < (MAX_SENSORS_IN_CS + 1); uart_num++) {
		mpu9250_detect_sensor(uart_num, &whoami);

		/* Valid sensor detected */
		if (whoami == MPU9250_VALID_WHO_AM_I) {
			set_uart_num(valid_num_sensors_in_cs, uart_num);
			set_sensor_type(valid_num_sensors_in_cs, whoami);

			sat_enable_sensor(valid_num_sensors_in_cs, get_uart_num(valid_num_sensors_in_cs));
		...
		{
			lsm6dsl_detect_sensor(uart_num, &whoami);
			if (whoami == LSM6DSL_WHOAMI)
			{
				set_uart_num(valid_num_sensors_in_cs, uart_num);
				set_sensor_type(valid_num_sensors_in_cs, whoami);
...
//	uint32_t lastRead = 0;
	...
	while (true) {
		...
		if (start_dumping == true) {
			...
			{
				sat_enable_sensor(sensor_num, get_uart_num(sensor_num));
				sensor_data[sensor_num].sensor_idx = sensor_num;
				...
				//Run sensor fusion
				retbool = get_raw_data_sample(sensor_num, &raw_data, &sensor_data[sensor_num]);
				...
				if (!retbool) {
					...
					sensor_data_valid |= 1 << sensor_num;
					...
					if (alternateSending == alternateSending_PERIOD) {
						if(globals.outputMode == OUTPUT_MODE_COUN && communicationTestCounterInitialization < 0) {
							communicationTestCounter[sensor_num][COMMUNICATIONCOUNTER_BUFF]++;
						}
					}
				}
...
#if PACE_SENSOR_OUTPUT
				if (alternateSending == alternateSending_PERIOD) {
					sensor_data_ts = k22f_get_rtc();

					for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
						sensor_data[sensor_num].sensor_idx = sensor_num; /* Local sensor index */
						sensor_data[sensor_num].timestamp = sensor_data_ts;
						...
						if (cbufPush(&sensorRecordQ, addSetMeasurement(0, sensor_data[sensor_num])) != E_OK) {
//							printf("error inserting to sensorRecordQ\r\n");
					...
					for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
						addSetMeasurement(1, sensor_data[sensor_num]);
					}
					alternateSending++;
				} else {
					 ...
					 */
					for (sensor_num = 0; sensor_num < valid_num_sensors_in_cs; sensor_num++) {
						addSetMeasurement(0, sensor_data[sensor_num]);
					}
					alternateSending = 1;
				}
#else
