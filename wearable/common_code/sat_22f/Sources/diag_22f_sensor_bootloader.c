...
#define SAT_ID			249 /* TODO later will read from memory region */
...
void processCommand(sat_mosi_packet_t packet) {
	...
	if (cmd & SAT_SPI_START) {
		...
		sensor_active = true;
	} else if(k22f_get_rtc() - timeStampOld > 20) { //CF to be sure it was commanded the stop dumping
		if(sensor_active)
...
int main(void) {
	...
	uint32_t satid = ts_read_from_flash(TS_FLASH_STARTING_ADDR);
	err_t flash_ret;
	uint8_t program_flash[4] = {0};

	program_flash[0] = satid & 0xFF;
	program_flash[1] = (satid & 0xFF00) >> 8;
	program_flash[2] = (satid & 0xFF0000) >> 16;
	program_flash[3] = (satid & 0xFF000000) >> 24;
	...
	satid = ts_read_from_flash(TS_FLASH_STARTING_ADDR); /* Should show 0xFF's after erase */

	program_flash[0]++;
	flash_ret = ts_write_to_flash(TS_FLASH_STARTING_ADDR, 4, program_flash);
	if (flash_ret != E_OK) {
		blinkLEDerror(redLed, 300);
	}

	while (1) {
		for (uint8_t qq = 0; qq < (program_flash[0] % 5); qq++) {
			turnOnLED(turqoiseLed);
			OSA_TimeDelay(500);
			turnOnLED(offLed);
			OSA_TimeDelay(500);
