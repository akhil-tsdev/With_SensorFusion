/*
 * mpu9250.c to be used with KSDK 1.3.0
 *
 *
...
//		}
//};
//static const gpio_output_pin_user_config_t mpu9250_cs1_pin =
//{
//		.pinName = kMpu9250Cs1Pin,
//		.config =
//		{
//				.outputLogic = 0,
//				.slewRate = kPortFastSlewRate,
...
//		}
//};
//static const gpio_output_pin_user_config_t mpu9250_cs2_pin =
//{
//		.pinName = kMpu9250Cs2Pin,
//		.config =
//		{
//				.outputLogic = 0,
//				.slewRate = kPortFastSlewRate,
...
//		}
//};
//static const gpio_output_pin_user_config_t mpu9250_cs3_pin =
//{
//		.pinName = kMpu9250Cs3Pin,
//		.config =
//		{
//				.outputLogic = 0,
//				.slewRate = kPortFastSlewRate,
...
//};
//
const dspi_device_t mpu9250_dspi_device =
{
	.bitsPerSec = 1000000L,
...
static const dspi_master_user_config_t mpu9250_dspi_master_user_config_cs1 =
{
	.whichCtar = kDspiCtar0,
	.whichPcs = kDspiPcs1,
	.pcsPolarity = kDspiPcs_ActiveLow,
...
static const dspi_master_user_config_t mpu9250_dspi_master_user_config_cs2 =
{
	.whichCtar = kDspiCtar0,
	.whichPcs = kDspiPcs2,
	.pcsPolarity = kDspiPcs_ActiveLow,
...
static const dspi_master_user_config_t mpu9250_dspi_master_user_config_cs3 =
{
	.whichCtar = kDspiCtar0,
	.whichPcs = kDspiPcs3,
	.pcsPolarity = kDspiPcs_ActiveLow,
...
static const dspi_master_user_config_t mpu9250_dspi_master_user_config_cs4 =
{
	.whichCtar = kDspiCtar0,
	.whichPcs = kDspiPcs4,
	.pcsPolarity = kDspiPcs_ActiveLow,
...
static const dspi_master_user_config_t mpu9250_dspi_master_user_config_cs5 =
{
	.whichCtar = kDspiCtar0,
	.whichPcs = kDspiPcs5,
	.pcsPolarity = kDspiPcs_ActiveLow,
...
static void mpu9250_write_reg(uint8_t reg_addr, uint8_t data)
{
	...
	readback = mpu9250_read_reg(reg_addr);
	if (readback != data)
	{
...
static void mpu9250_write_reg_no_verify(uint8_t reg_addr, uint8_t data)
{
	dspi_status_t Error;
...
//	static uint32_t counter = 0;
	...
	while(1) {
		Error = DSPI_DRV_MasterTransferBlocking(kMpu9250SpiInstance, &mpu9250_dspi_device,
...
{
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_DO,   data);
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_ADDR, 0x0c);      // write, magnetometer address
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_REG,  reg_addr);
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_CTRL, 0x80+(MAG_SUBSAMPLE-1));      // enable

	OSA_TimeDelay(30);			// delay for write to complete, should happen every 10 ms

	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_CTRL, (MAG_SUBSAMPLE-1));      // disable
...
//	uint8_t data;
//
//	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_ADDR, 0x8c);      // read, magnetometer address
//	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_REG,  reg_addr);
//	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_CTRL, 0x80+(MAG_SUBSAMPLE-1));      // enable
//
//	OSA_TimeDelay(30);			// delay for reaad to complete, should happen every 10 ms
//
//	mpu9250_read_regs(MPU9250_REG_I2C_SLV0_DI, 1, & data);
//
//	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_CTRL, (MAG_SUBSAMPLE-1));      // disable
//
...
static uint8_t read_mag_reg_fast(uint8_t reg_addr)
{
	...
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_ADDR, 0x8c);      // read, magnetometer address
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_REG,  reg_addr);
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_CTRL, 0x80+(MAG_SUBSAMPLE-1));      // enable

	OSA_TimeDelay(10);			// delay for reaad to complete, should happen every 10 ms

	mpu9250_read_regs(MPU9250_REG_I2C_SLV4_DI, 1, & data);
...
//	GPIO_DRV_OutputPinInit(& mpu9250_cs1_pin);
//	GPIO_DRV_OutputPinInit(& mpu9250_cs2_pin);
//	GPIO_DRV_OutputPinInit(& mpu9250_cs3_pin);
//
//	GPIO_DRV_OutputPinInit(& mpu9250_mosi_pin);

//	configure_spi_pins(kMpu9250SpiInstance);
//	configure_spi_pins(kSensor1);
//	configure_spi_pins(kSensor2);
//	configure_spi_pins(kSensor3);
 ...
 */
void mpu9250_set_csn(uint16_t cs, uint16_t val) {
...
void mpu9250_init_cs_gpio(void) {
	...
	cs0.config.outputLogic = 1;
	cs0.config.slewRate = kPortSlowSlewRate;
//	cs0.config.driveStrength = kPortHighDriveStrength;
	cs0.pinName = GPIO_MAKE_PIN(GPIOC_IDX, 4);
	GPIO_DRV_OutputPinInit(&cs0);
...
//	cs1.config.driveStrength = kPortHighDriveStrength;
	...
	cs2.config.outputLogic = 1;
	cs2.config.slewRate = kPortSlowSlewRate;
//	cs2.config.driveStrength = kPortHighDriveStrength;
	cs2.pinName = GPIO_MAKE_PIN(GPIOD_IDX, 5);
	GPIO_DRV_OutputPinInit(&cs2);

	cs3.config.outputLogic = 1;
	cs3.config.slewRate = kPortSlowSlewRate;
//	cs3.config.driveStrength = kPortHighDriveStrength;
	cs3.pinName = GPIO_MAKE_PIN(GPIOD_IDX, 6);
	GPIO_DRV_OutputPinInit(&cs3);

	cs4.config.outputLogic = 1;
	cs4.config.slewRate = kPortSlowSlewRate;
//	cs4.config.driveStrength = kPortHighDriveStrength;
	cs4.pinName = GPIO_MAKE_PIN(GPIOC_IDX, 0);
	GPIO_DRV_OutputPinInit(&cs4);

	cs5.config.outputLogic = 1;
	cs5.config.slewRate = kPortSlowSlewRate;
//	cs5.config.driveStrength = kPortHighDriveStrength;
	cs5.pinName = GPIO_MAKE_PIN(GPIOB_IDX, 23);
	GPIO_DRV_OutputPinInit(&cs5);
}
...
bool mpu9250_detect_sensor(uint8_t cs, uint8_t* whoami) {
	...
	*whoami = 0;
	...
	for (uint8_t qq = 0; qq < MAX_SPI_CS; qq++) {
		if (qq == cs) {
			mpu9250_set_csn(qq, 0);
		} else {
			mpu9250_set_csn(qq, 1);
		}
	}
	switch (cs) {
		case 0:
			DSPI_DRV_MasterDeinit(kMpu9250SpiInstance);
			status = DSPI_DRV_MasterInit(kMpu9250SpiInstance, & mpu9250_dspi_master_state, & mpu9250_dspi_master_user_config);
			break;
		case 1:
			DSPI_DRV_MasterDeinit(kMpu9250SpiInstance);
			status = DSPI_DRV_MasterInit(kMpu9250SpiInstance, & mpu9250_dspi_master_state, & mpu9250_dspi_master_user_config_cs1);
			break;
		case 2:
			DSPI_DRV_MasterDeinit(kMpu9250SpiInstance);
			status = DSPI_DRV_MasterInit(kMpu9250SpiInstance, & mpu9250_dspi_master_state, & mpu9250_dspi_master_user_config_cs2);
			break;
		case 3:
			DSPI_DRV_MasterDeinit(kMpu9250SpiInstance);
			status = DSPI_DRV_MasterInit(kMpu9250SpiInstance, & mpu9250_dspi_master_state, & mpu9250_dspi_master_user_config_cs3);
			break;
		case 4:
			DSPI_DRV_MasterDeinit(kMpu9250SpiInstance);
			status = DSPI_DRV_MasterInit(kMpu9250SpiInstance, & mpu9250_dspi_master_state, & mpu9250_dspi_master_user_config_cs4);
			break;
		case 5:
			DSPI_DRV_MasterDeinit(kMpu9250SpiInstance);
			status = DSPI_DRV_MasterInit(kMpu9250SpiInstance, & mpu9250_dspi_master_state, & mpu9250_dspi_master_user_config_cs5);
			break;
		default:
			break;
	}
	if (status != kStatus_DSPI_Success) {
		printf("status=%d Failed to initialize sensor %d", status, cs);
		return false;
	}

#if TMP_DEBUG
	printf("\n\rDEBUG: Init sensor %d successful!", cs);
#endif

	uint8_t data[MPU9250_REG_COUNT];

	mpu9250_read_regs(MPU9250_REG_WHO_AM_I, 1, data);
	*whoami = data[0];
...
bool mpu9250_enable_sensor(uint8_t sensor_num) {
	int status = kStatus_DSPI_Timeout;
	uint8_t cs = valid_cs[sensor_num];
...
bool mpu9250_init_regs(void) {
	mpu9250_write_reg_no_verify(MPU9250_REG_SIGNAL_PATH_RESET,  0x07);  // signal path reset = gyro_rst + accel_rst + tmp_rst
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x07);  // fifo_rst + i2c_mst_rst + sig_cond_rst
								 	 	 	 	 	  // fifo_rst and i2c_mst_rst auto-clear after one clock cycle
													  // sig_cond_rst might also auto-clear; doc is unclear
	OSA_TimeDelay(10);
	mpu9250_write_reg_no_verify(MPU9250_REG_SIGNAL_PATH_RESET,  0x00);  // clear signal path reset

	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_CTRL,      0x00);  // disable
	mpu9250_write_reg(MPU9250_REG_I2C_SLV1_CTRL,      0x00);  // disable
	mpu9250_write_reg(MPU9250_REG_I2C_SLV2_CTRL,      0x00);  // disable
	mpu9250_write_reg(MPU9250_REG_I2C_SLV3_CTRL,      0x00);  // disable
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_CTRL,      0x00);  // disable
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x20);  // I2C_MST_EN

	mpu9250_write_reg(MPU9250_REG_SMPLRT_DIV,         SMPLRT_DIV);  // sample rate divisor = 10, so 100 Hz output
	mpu9250_write_reg(MPU9250_REG_CONFIG,             0x00 | DPLF_CFG);  // FIFO_MODE disabled, EXT_SYNC_SET disabled, DPLF_CFG
	mpu9250_write_reg(MPU9250_REG_GYRO_CONFIG,        (GYRO_FS_SEL<<3) | FCHOICE_B);  // gyro self-test disabled, all axes
	mpu9250_write_reg(MPU9250_REG_ACCEL_CONFIG,       ACCEL_FS_SEL<<3);  // accel self-test disabled, all axes
	mpu9250_write_reg(MPU9250_REG_ACCEL_CONFIG2,      (ACCEL_FCHOICE_B<<3) | A_DPLF_CFG);
	mpu9250_write_reg(MPU9250_REG_PWR_MGMT_1,         0x00);  // PWR_MGMT_1 = 0

#if MAG_SUBSAMPLE != 1
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_CTRL,      (MAG_SUBSAMPLE-1));  // I2C_MST_DLY: only read every nth sample
	mpu9250_write_reg(MPU9250_REG_I2C_MST_DELAY_CTRL, 0x81);  // DELAY_ES_SHADOW | I2C_SLV0_DLY_EN
#else
	mpu9250_write_reg(MPU9250_REG_I2C_SLV4_CTRL,      0x00);  // I2C_MST_DLY: read every sample
	mpu9250_write_reg(MPU9250_REG_I2C_MST_DELAY_CTRL, 0x80);  // DELAY_ES_SHADOW
#endif

	mpu9250_write_reg(MPU9250_REG_I2C_MST_CTRL,       0x10);  // not MULT_MST_EN, !WAIT_FOR_ES, !SLV_3_FIFO_EN, I2C_MST_P_NSR, I2C_MST_CLK = 348 kHz

#if READ_MAG_DIRECTLY
	///////////////////////cwati mag setup
	mpu9250_write_reg(MPU9250_REG_INT_PIN_CFG, 0x2);  // bypass mode cwati
#endif /* READ_MAG_DIRECTLY */

	mpu9250_write_reg(MPU9250_REG_INT_PIN_CFG, 0xc0); /* INT active low, open drain */
	mpu9250_write_reg(MPU9250_REG_INT_ENABLE, 0x00);  /* DIsable all INT */
	...
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x00);  // !I2C_MST_EN
...
bool mpu9250_start(void)
{
	...
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x00);  // !I2C_MST_EN
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x02);  // !I2C_MST_EN | I2C_MST_RST
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x00);  // !I2C_MST_EN
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x20);  // I2C_MST_EN
	...
	mpu9250_write_reg(MPU9250_REG_SMPLRT_DIV,         smplrt_div);  // sample rate divisor = 10, so 100 Hz output
//
//	uint8_t tmp = 	mpu9250_read_reg(MPU9250_REG_SMPLRT_DIV);  // sample rate divisor = 10, so 100 Hz output

	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_ADDR,      0x80 + AK8963_I2C_ADDR);  // read from 0x0c
	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_REG,       AK8963_FIRST_REG);
	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_CTRL,      0x80 + AK8963_REG_COUNT);  // enable auto read, length
#endif /* READ_MAG_DIRECTLY */
...
static bool mpu9250_self_test_start(void)
{
	...
	mpu9250_write_reg_no_verify(MPU9250_REG_USER_CTRL,          0x00);  // !I2C_MST_EN
	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_CTRL,      0x00);  // disable reading magnetometer

	mpu9250_write_reg(MPU9250_REG_SMPLRT_DIV,         SELFTEST_SMPLRT_DIV);  // sample rate divisor = 1, so 1000 Hz output
...
//	mpu9250_write_reg(MPU9250_REG_INT_ENABLE,         0x00);  // disable data ready interrupt
	mpu9250_write_reg(MPU9250_REG_SMPLRT_DIV,         SMPLRT_DIV);  // sample rate divisor = 10, so 100 Hz output
	mpu9250_write_reg(MPU9250_REG_I2C_SLV0_CTRL,      0x00);  // disable reading magnetometer
	return true;
}
...
//	uint8_t data;
//
//	mpu9250_read_regs(MPU9250_REG_INT_STATUS, 1, &data);
//
//	if (! data & 0x01) {
//		return false;
//	}
...
bool mpu9250_get_factory_self_test_results(mpu9250_self_test_results_t *factory_st)
{
	if (running[mpu9250_num])
		return false;
	mpu9250_read_regs(MPU9250_REG_SELF_TEST_X_GYRO, 3, factory_st->gyro);
	mpu9250_read_regs(MPU9250_REG_SELF_TEST_X_ACCEL, 3, factory_st->accel);
	return true;
}
		               ...
		               mpu9250_average_data_t *selftest)
{
	...
	mpu9250_write_reg(MPU9250_REG_SMPLRT_DIV,         SELFTEST_SMPLRT_DIV);
	mpu9250_write_reg(MPU9250_REG_CONFIG,             0x00 | SELFTEST_DPLF_CFG);  // FIFO_MODE disabled, EXT_SYNC_SET disabled, DPLF_CFG
	mpu9250_write_reg(MPU9250_REG_GYRO_CONFIG,        (SELFTEST_GYRO_FS_SEL<<3) | SELFTEST_FCHOICE_B);  // gyro self-test disabled, all axes
	mpu9250_write_reg(MPU9250_REG_ACCEL_CONFIG,       (SELFTEST_ACCEL_FS_SEL<<3));  // accel self-test disabled, all axes
	mpu9250_write_reg(MPU9250_REG_ACCEL_CONFIG2,      (SELFTEST_ACCEL_FCHOICE_B<<3) | SELFTEST_A_DPLF_CFG);

	status &= average_test_samples(flags, 200, normal);

	// XXX set self-test bits
	mpu9250_write_reg(MPU9250_REG_GYRO_CONFIG,        0xe0 | (SELFTEST_GYRO_FS_SEL<<3) | SELFTEST_FCHOICE_B);  // gyro self-test enabled, all axes
	mpu9250_write_reg(MPU9250_REG_ACCEL_CONFIG,       0xe0 | (SELFTEST_ACCEL_FS_SEL<<3));  // accel self-test enabled, all axes
...
{
	mpu9250_write_reg(MPU9250_REG_PWR_MGMT_1, 0x4F); // SLEEP=1, PD_PTAT=1 (power down volt gen), stop clock
//	mpu9250_write_reg(MPU9250_REG_PWR_MGMT_2, 0x3F); // disable accel and gyro
}

void mpu9250_wakeup(void)
{
	mpu9250_write_reg(MPU9250_REG_PWR_MGMT_1, 0x00); // enable voltage generator and clock
//	mpu9250_write_reg(MPU9250_REG_PWR_MGMT_2, 0x00); // enable accel and gyro
}
