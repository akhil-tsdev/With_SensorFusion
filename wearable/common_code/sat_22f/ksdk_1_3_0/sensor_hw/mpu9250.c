...
//	int resulting_compass_rate;
	...
	PORT_HAL_SetMuxMode(PORTC, 8U, kPortMuxAsGpio); /* mpu9250_int_pin */
	GPIO_DRV_InputPinInit(& mpu9250_int_pin);

#if 1
	mpu9250_write_reg(MPU9250_REG_INT_PIN_CFG,        0xb0);  // int active low, push-pull, latched, cleared by any read
    												  //    FSYNC pin does not cause interrupt, no I2C bypass
#else
	mpu9250_write_reg(MPU9250_REG_INT_PIN_CFG,        0x80);  // int active low, push-pull, 50 us pulse, cleared by reading INT_STATUS
    												  //    FSYNC pin does not cause interrupt, no I2C bypass
#endif
	mpu9250_write_reg(MPU9250_REG_INT_ENABLE,         0x00);  // disable all interrupts
	GPIO_DRV_InputPinInit(& mpu9250_int_pin);
...
bool mpu9250_start(void)
{
	...
	mpu9250_write_reg(MPU9250_REG_INT_ENABLE,         0x01);  // enable data ready interrupt
