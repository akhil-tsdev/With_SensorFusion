...
int communicationTestCounter[MAX_SENSORS][2] = {0};
sensor_data_t *local_raw_data;


static void copy_raw_data(sensor_data_t *raw_data, sensor_record_t *sensor_data);
static void copy_sensor_fusion_output(sat_sensor_record_t *sensor_record, struct MagCalibration *pthisMagCal);
...
void process_sensor_fusion(sensor_data_t *raw_data, sat_sensor_record_t* sensor_record) {
    ...
    globals.RunKF_Event_Flag[mpu9250_num] = 0;
    ...
    if (globals.RunKF_Event_Flag[mpu9250_num] || globals.newKF_enabled)
    {
    	globals.MagCal_Event_Flag[mpu9250_num] = 0;
    	...
    	if (globals.MagCal_Event_Flag[mpu9250_num] && (globals.outputMode == OUTPUT_MODE_BMAG || globals.enable_MagBiasCalibrationAlways) && !globals.disable_magCalibrationAlways) //HWE ALIGNMENT CONFIGURATION MAG ENABLED 2
    		MagCal_Run(&thisMagCal[mpu9250_num], &thisMagBuffer[0]);
    }
    ...
    if (mpu9250_num == 0)
    {
    	...
    	static uint32_t timepast = 0;

    	mycounter = mycounter+1;

    	//sensor_record->data.accel_x = mycounter;
    	//sensor_record->data.accel_y = (uint16_t) k22f_get_rtc();

    	while ((k22f_get_rtc() - timepast) < (TCs2*1000-1))
    	{
    		OSA_TimeDelay(1);
    	}

    	//sensor_record->data.accel_z = (uint16_t) (k22f_get_rtc() - timepast);
    	timepast = k22f_get_rtc();

    }

    // Copy Sensor Fusion output
    copy_sensor_fusion_output(sensor_record, &thisMagCal[mpu9250_num]);
...
static void copy_sensor_fusion_output(sat_sensor_record_t *sensor_record, struct MagCalibration *pthisMagCal)
{
	sensor_record_t* sensor_data = &(sensor_record->data);
	...
	if (globals.outputMode == OUTPUT_MODE_QUAT || globals.outputMode == OUTPUT_MODE_QUMA || globals.outputMode == OUTPUT_MODE_MAGO)
	{
		...
		sensor_data->accel_x = (int16_t)(communicationTestCounter[sensor_record->sensor_idx][COMMUNICATIONCOUNTER_BUFF]%CPI_QUATERNION16_COUNTER_MAXVALU);
		sensor_data->accel_y = (int16_t)(communicationTestCounter[sensor_record->sensor_idx][COMMUNICATIONCOUNTER_SPIP]%CPI_QUATERNION16_COUNTER_MAXVALU);
