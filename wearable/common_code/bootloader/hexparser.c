#include "hexparser.h"

#include <stdio.h>
#include <stdint.h>

static inline uint8_t nibble_from_hex_char(char c)
{
    switch (c)
    {
        case '0':
        case '1':
        case '2':
        case '3':
        case '4':
        case '5':
        case '6':
        case '7':
        case '8':
        case '9':
            return c - '0';

        case 'a':
        case 'b':
        case 'c':
        case 'd':
        case 'e':
        case 'f':
            return c - 'a' + 10;

        case 'A':
        case 'B':
        case 'C':
        case 'D':
        case 'E':
        case 'F':
            return c - 'A' + 10;

    }
    return 0;
}

static inline uint8_t uint8_from_hex_char(const char c[2])
{
    return nibble_from_hex_char(c[0]) << 4 | 
           nibble_from_hex_char(c[1]);
}

static inline hexparser_record_type hexparser_record_type_from_uint8(const uint8_t t)
{
	switch (t) {
		case 0:
			return DATA_RECORD;
		case 1:
			return END_OF_FILE_RECORD;
		case 2:
			return EXTENDED_SEGMENT_ADDRESS_RECORD;
		case 3:
			return START_SEGMENT_ADDRESS_RECORD;
		case 4:
			return EXTENDED_LINEAR_ADDRESS_RECORD;
		case 5:
			return START_LINEAR_ADDRESS_RECORD;
		default:
			return INVALID_RECORD;
	}
}

static inline uint16_t uint16_from_hex_char(const char c[4])
{
    return nibble_from_hex_char(c[0]) << 12 | 
           nibble_from_hex_char(c[1]) << 8 | 
           nibble_from_hex_char(c[2]) << 4 | 
           nibble_from_hex_char(c[3]); 
}

static inline uint16_t uint16_from_hex_bytes(const char b[2])
{
	return ((uint16_t)b[0] << 8) | ((uint16_t)b[1]);
}

static inline uint32_t data_uint32_from_hex_char(const char c[8])
{
    return nibble_from_hex_char(c[7]) << 24 | 
           nibble_from_hex_char(c[6]) << 28 | 
           nibble_from_hex_char(c[5]) << 16 | 
           nibble_from_hex_char(c[4]) << 20 |
           nibble_from_hex_char(c[3]) << 8 | 
           nibble_from_hex_char(c[2]) << 12 | 
           nibble_from_hex_char(c[1]) << 0 | 
           nibble_from_hex_char(c[0]) << 4; 
}

static inline uint32_t data_uint32_from_hex_bytes(const char b[4])
{
	return  ((uint32_t)b[3] << 24) |
			((uint32_t)b[2] << 16) |
			((uint32_t)b[1] << 8) |
			((uint32_t)b[0]);
}

bool hexparser_parse_bytes(const char *record_bytes, uint8_t length, hexparser_record * record)
{
	record->type = hexparser_record_type_from_uint8((uint8_t)(record_bytes[3]));
	record->address = uint16_from_hex_bytes(&record_bytes[1]);
	record->byte_count = (uint8_t)(record_bytes[0]);

	record->checksum = (uint8_t)(record_bytes[4+(record->byte_count)]);

	switch (record->type)
	{
		case DATA_RECORD:
			for (uint8_t i = 0; i < (record->byte_count / 4); i++)
			{
				record->data.words[i] = data_uint32_from_hex_bytes(&record_bytes[4+i*4]);
			}
			break;

		case EXTENDED_LINEAR_ADDRESS_RECORD:
		case EXTENDED_SEGMENT_ADDRESS_RECORD:
			record->data.words[0] = uint16_from_hex_bytes(&record_bytes[4]);
			break;

		case START_SEGMENT_ADDRESS_RECORD:
		case START_LINEAR_ADDRESS_RECORD:
			for (uint8_t i = 0; i < (record->byte_count / 4); i++)
			{
				record->data.words[i] = data_uint32_from_hex_bytes(&record_bytes[4+i*4]);
			}
			break;

		default:
			break;
	}

	return true;
}

bool hexparser_parse_string(const char * record_string, uint8_t length, hexparser_record * record)
{
    record->type = hexparser_record_type_from_uint8(uint8_from_hex_char(&record_string[7]));
    record->address = uint16_from_hex_char(&record_string[3]);
    record->byte_count = uint8_from_hex_char(&record_string[1]);

    record->checksum = uint8_from_hex_char(&record_string[9+(record->byte_count*2)]);

    switch (record->type)
    {
        case DATA_RECORD:
            for (uint8_t i = 0; i < (record->byte_count / 4); i++) 
            {
                record->data.words[i] = data_uint32_from_hex_char(&record_string[9+i*8]);
            }
            break;

        case EXTENDED_LINEAR_ADDRESS_RECORD:
        case EXTENDED_SEGMENT_ADDRESS_RECORD:
            record->data.words[0] = uint16_from_hex_char(&record_string[9]);
            break;

        case START_SEGMENT_ADDRESS_RECORD:
        case START_LINEAR_ADDRESS_RECORD:
            for (uint8_t i = 0; i < (record->byte_count / 4); i++) 
            {
                record->data.words[i] = data_uint32_from_hex_char(&record_string[9+i*8]);
            }
            break;

        default:
            break;
    }

    return true;
}

bool hexparser_is_record_valid(hexparser_record * record)
{
    uint8_t sum = 0;
    sum += record->byte_count;
    sum += record->address & 0x00FF;
    sum += (record->address & 0xFF00) >> 8;
    sum += record->type;

    for (uint8_t i = 0; i < record->byte_count; i++)
    {
        sum += record->data.bytes[i];
    }

    return (record->checksum == ((0x100 - sum) & 0xFF));
}
